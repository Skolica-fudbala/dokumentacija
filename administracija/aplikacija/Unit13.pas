...
begin
  ...
  if (cbTipoviRoditelja.Items.Count >= 1) then
     begin
      roditeljId := intToStr(NativeInt(cbTipoviRoditelja.Items.Objects[cbTipoviRoditelja.ItemIndex]))
     end;
  ...
  with DM.qryRoditeljaZaDete do
    begin
      ...
      sql.Text := 'SELECT *' +
        'FROM roditelji r ' +
        'INNER JOIN clanovi_roditelji cr ON cr.roditelj_id = r.id ' +
        'WHERE cr.clan_id = ' + edtClanId.Text + ' AND cr.roditelj_id = ' + roditeljId;
      Active := true;
    end;
end;
