...
begin
  with DM.qryDodajDeteRoditelju do
    begin
      ...
      if (DM.qryRoditelji.fieldByName('pol').AsString.Equals('M')) then
        begin
          Params[2].AsString := 'OTAC';
        end
        else
          begin
            Params[2].AsString := 'MAJKA';
          end;

      ExecSQL;
    end;

    ShowMessage('Uspešno ste dodali novo dete roditelju!');
...
begin
  ...
  ShowMessage('Uspešno ste obrisali dete roditelju!');
end;
...
// Procedura koja osvežava combobox sa decom kojima roditelj nije izabran.
   ...
   with DM.qryClanoviBezRoditelja do
    begin
      ...
      if (DBComboBox1.Text <> 'M') then
        begin
          sql.Text := 'SELECT id, (ime || " " || prezime) AS ime FROM clanovi WHERE id NOT IN (SELECT clan_id FROM clanovi_roditelji WHERE tip != "OTAC")';
        end
      else
        begin
          sql.Text := 'SELECT id, (ime || " " || prezime) AS ime FROM clanovi WHERE id NOT IN (SELECT clan_id FROM clanovi_roditelji WHERE tip != "MAJKA")';
        end;
...
begin
  ...
  with DM.qryDeca do
    begin
      Active := false;
      sql.Clear;
      sql.Text := 'SELECT c.id, (c.ime || " " || c.prezime) AS ime, tip ' +
    ...
    while not DM.qryDeca.Eof do
      begin
        lbDeca.Items.AddObject(
          DM.qryDeca.FieldByName('ime').AsString + ' (' + lowercase(DM.qryDeca.FieldByName('tip').AsString) + ')',
          TObject(DM.qryDeca.FieldByName('id').AsInteger)
        );
